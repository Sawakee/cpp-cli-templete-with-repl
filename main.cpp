...
struct Command {
  ...
  void add_subcommand(std::shared_ptr<Command> sub) { subcommands[sub->name] = std::move(sub); }
  ...
  int execute(const std::vector<std::string>& tokens, size_t index = 0,
...
// 現在位置のパス文字列化
std::string build_path(const std::shared_ptr<Command>& cmd,
